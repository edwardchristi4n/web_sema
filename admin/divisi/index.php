            ...
            <header class="sticky top-0 z-10 border-b border-white/10 bg-night/70 backdrop-blur-xl">
                <div class="px-6 py-5 flex flex-wrap gap-4 items-center justify-between">
                    <div>
                        ...
                        <h1 class="font-display text-2xl md:text-3xl font-semibold">Kelola Divisi</h1>
                        <p class="text-sm text-white/60">Tambah, perbarui, atau hapus data divisi</p>
                    </div>
                    <?php if ($editDivisi): ?>
            ...
            <main class="px-6 py-8 space-y-8">
                ...
                <section class="glass-panel rounded-[2rem] p-6">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <h2 class="text-2xl font-semibold text-white"><?php echo $editDivisi ? 'Perbarui Divisi' : 'Tambah Divisi'; ?></h2>
                        </div>
                    </div>
                    <form action="index.php" method="post" enctype="multipart/form-data" class="space-y-5">
                        ...
                        <div class="grid md:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-sm font-semibold text-white/80 mb-2">Nama Divisi <span class="text-rose-300">*</span></label>
                                <input type="text" name="nama_divisi" value="<?php echo $editDivisi ? h($editDivisi['nama_divisi']) : ''; ?>" class="w-full px-4 py-3 rounded-2xl border border-white/10 focus:outline-none focus:ring-2 focus:ring-tide/40 bg-white/5" required>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-white/80 mb-2">Nama Koordinator</label>
                                <input type="text" name="nama_koor" value="<?php echo $editDivisi ? h($editDivisi['nama_koor'] ?? '') : ''; ?>" placeholder="Nama koordinator bidang" class="w-full px-4 py-3 rounded-2xl border border-white/10 focus:outline-none focus:ring-2 focus:ring-tide/40 bg-white/5">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-white/80 mb-2">Foto Koordinator</label>
                            <?php if ($editDivisi && !empty($editDivisi['foto_koor'])): ?>
                                <div class="mb-3 flex items-center gap-3">
                                    <img src="<?php echo h($editDivisi['foto_koor']); ?>" class="w-16 h-16 rounded-xl object-cover border border-white/10">
                                    <span class="text-sm text-white/50">Foto saat ini. Upload baru untuk mengganti.</span>
                                </div>
                            <?php endif; ?>
                            <input type="file" name="foto_koor" accept="image/*" class="w-full px-4 py-3 rounded-2xl border border-white/10 bg-white/5 text-sm">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-white/80 mb-2">Visi</label>
                            <textarea name="visi" rows="3" placeholder="Tuliskan visi bidang ini..." class="w-full px-4 py-3 rounded-2xl border border-white/10 focus:outline-none focus:ring-2 focus:ring-tide/40 bg-white/5 resize-none"><?php echo $editDivisi ? h($editDivisi['visi'] ?? '') : ''; ?></textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-white/80 mb-2">Misi</label>
                            <textarea name="misi" rows="3" placeholder="Tuliskan misi bidang ini..." class="w-full px-4 py-3 rounded-2xl border border-white/10 focus:outline-none focus:ring-2 focus:ring-tide/40 bg-white/5 resize-none"><?php echo $editDivisi ? h($editDivisi['misi'] ?? '') : ''; ?></textarea>
                        </div>

                        <button type="submit" class="w-full px-4 py-3 rounded-2xl font-semibold text-night bg-gradient-to-r from-ember to-tide hover:shadow-[0_0_30px_rgba(241,90,41,0.35)] transition">
                ...
                <section class="glass-panel rounded-[2rem] p-6">
                    <div class="mb-4">
                        ...
                        <h2 class="text-xl font-semibold text-white">Divisi Terdaftar</h2>
                    </div>
                        ...
                        <table class="min-w-full text-sm text-white/80">
                            ...
                            <tbody class="divide-y divide-white/10">
                                ...
                                <tr class="odd:bg-white/5 hover:bg-white/10">
                                    <td class="py-3 font-semibold text-white"><?php echo h($divisi['nama_divisi']); ?></td>
                                    <td class="py-3 text-white/60">
                                        <?php if (!empty($divisi['foto_koor'])): ?>
                                            <div class="flex items-center gap-2">
                                                <img src="<?php echo h($divisi['foto_koor']); ?>" class="w-8 h-8 rounded-full object-cover border border-white/10">
                                    ...
                                    <td class="py-3 text-white/60">
                                        <?php echo !empty($divisi['visi']) ? '<span class="px-2 py-0.5 bg-emerald-500/10 text-emerald-200 rounded-full text-xs border border-emerald-500/30">Lengkap</span>' : '<span class="px-2 py-0.5 bg-white/10 text-white/60 rounded-full text-xs border border-white/10">Belum diisi</span>'; ?>
                                    </td>
                                    <td class="py-3 text-right space-x-2">
                                        <a href="../../bidang-detail.php?id=<?php echo (int)$divisi['id_divisi']; ?>" target="_blank" class="text-sm font-semibold text-white/50 hover:text-white">Preview</a>
                                        <a href="index.php?edit_divisi=<?php echo (int)$divisi['id_divisi']; ?>" class="text-sm font-semibold text-tide hover:underline">Edit</a>
                                ...
                                <?php endforeach; ?>
                                <?php if (!$divisiList): ?>
                                <tr><td colspan="4" class="py-4 text-center text-white/50">Belum ada data divisi.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
