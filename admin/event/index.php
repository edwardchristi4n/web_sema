    ...
    <header class="bg-white border-b border-orange-100 shadow-sm sticky top-0 z-10">
        <div class="max-w-6xl mx-auto px-4 py-4 flex flex-wrap gap-4 items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-semibold text-slate-900">Kelola Event</h1>
                <p class="text-sm text-slate-500">Atur agenda kampus dan organisasi</p>
            </div>
            ...
            <form action="index.php" method="post" class="grid gap-4" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="judul" value="<?php echo $editEvent ? h($editEvent['judul']) : ''; ?>" class="w-full px-4 py-3 rounded-2xl border border-orange-100 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-orange-50" required />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Tanggal</label>
                        <input type="date" name="tanggal" value="<?php echo $editEvent ? h($editEvent['tanggal']) : ''; ?>" class="w-full px-4 py-3 rounded-2xl border border-orange-100 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-orange-50" required />
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Deskripsi</label>
                    <textarea name="deskripsi" rows="3" class="w-full px-4 py-3 rounded-2xl border border-orange-100 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-orange-50"><?php echo $editEvent ? h($editEvent['deskripsi']) : ''; ?></textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Foto Event</label>
                    <input type="file" name="foto_event" accept="image/*" class="w-full px-4 py-3 rounded-2xl border border-orange-100 bg-orange-50 focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    <p class="text-xs text-slate-500 mt-1">Format JPG, PNG, WEBP &bull; Maks 2MB</p>
                    <?php if ($editEvent && $editEvent['foto_event']): ?>
                        <p class="text-xs text-slate-500 mt-1">File saat ini: <?php echo h($editEvent['foto_event']); ?></p>
                    <?php endif; ?>
                </div>
        ...
        <section class="bg-white border border-orange-100 rounded-3xl p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <h2 class="text-xl font-semibold text-slate-900">Event Terdaftar</h2>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-orange-50">
                        <?php foreach ($eventList as $event): ?>
                            <tr class="hover:bg-orange-50/60">
                                <td class="py-3">
                                    ...
                                    <p class="text-xs text-slate-500 line-clamp-2"><?php echo h($event['deskripsi']); ?></p>
                                </td>
                                <td class="py-3 text-sm text-slate-700"><?php echo h($event['tanggal']); ?></td>
                                <td class="py-3 text-xs text-slate-500"><?php echo h($event['foto_event']); ?></td>
                                <td class="py-3 text-right space-x-2">
                                    <a href="index.php?edit_event=<?php echo (int)$event['id_event']; ?>" class="text-sm font-semibold text-orange-600 hover:underline">Edit</a>
                        ...
                        <?php if (!$eventList): ?>
                            <tr>
                                <td colspan="4" class="py-4 text-center text-slate-500">Belum ada event terdaftar.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
